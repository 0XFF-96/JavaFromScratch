public class No36{
    ...
    private static int getInverseParis(int[] arr){
        if (arr == null)
            return 0;
        int[] clone = arr.clone();
    ...
    private static int mergeSort(int[] array, int[] result, int start, \
                    int end){
        ...
        while (leftIndex >= start && rightIdex >= start + length +1){
            if (array[left[Index] > array[rightIndex]){
